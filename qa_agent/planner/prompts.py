...
def planner_system_prompt() -> str:
    return (
        ...
        "- 패키지명에서 앱 이름을 추측하지 마라. 앱 이름을 모르면 '대상 앱' 또는 '앱'이라고만 표기하라\n"
        ...
        "- 예: adb_press_button의 경우 'key_code'가 아니라 'button'을 사용해야 한다 (예: {\"button\": \"back\"})\n"
        "- 각 tool의 parameters에서 required로 표시된 인자는 반드시 포함해야 한다\n"
        "- 가능하면 초반에 take_screenshot을 포함해 관측 기반으로 시작\n"
        "- 화면 확인 혹은 버튼 확인은 smart find 으로만 확인\n"
